...
static char capslock_state; // 大写锁定
static char scrlock_state;  // 滚动锁定
static char numlock_state;  // 数字锁定
static char extcode_state;  // 扩展码状态
...
void keyboard_handler(u32 vector)
{
    ...
    // 是否是断码，按键抬起
    char breakcode = ((scancode & 0x0080) != 0);
    if (breakcode)
    {
        // 如果是则设置状态
